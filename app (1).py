#!/usr/bin/env python3
"""
W26 (WORLD CUP 2026) Price Monitor -> Notifiche Discord
...
prezzo ogni 15s + notifiche via Discord webhook) gira in un thread in
background.
"""
...
DISCORD_WEBHOOK_URL = os.environ.get("DISCORD_WEBHOOK_URL", "INSERISCI_QUI_L_URL_DEL_WEBHOOK")
...
def send_notification(message: str, title: str = "W26 Alert", priority: str = "default") -> None:
    ...
    emoji_map = {"urgent": "🚨", "high": "📈", "default": "⚠️"}
    prefix = emoji_map.get(priority, "")
    content = f"{prefix} **{title}**\n{message}"
    try:
        r = requests.post(DISCORD_WEBHOOK_URL, json={"content": content}, timeout=10)
        if r.status_code not in (200, 204):
            print(f"[{now()}] ERRORE invio Discord: {r.status_code} - {r.text}")
    except requests.RequestException as e:
        print(f"[{now()}] ERRORE connessione Discord: {e}")
